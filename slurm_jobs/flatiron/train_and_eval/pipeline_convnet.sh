#!/bin/bash
PARAM_FILE="wider_convnet.txt"
SAVE_FOLDER="results/"
MODEL_NAME=("convnet_wider_cifar") #mode of the model, can be "standard" or "convnet" or "convnet2" //// "vone_convnet-layer1_norm" or vone_convnet-layer1_norm-relu_last
...
# WEIGHT_DECAYS=("0.0 0.0005 0.005 0.05")
						...
						for EP in ${EPS[@]};do
							for WEIGHT_DECAY in ${WEIGHT_DECAYS[@]};do
								for WIDTH_SCALE in ${WIDTH_SCALES[@]};do
									echo "${SAVE_FOLDER} ${FRONTEND} ${NORM_POSITION} ${MODEL} ${SEED} ${MODE} ${NORMALIZE} ${EP} ${WEIGHT_DECAY} ${WIDTH_SCALE}" >> ${PARAM_FILE}
								done
							done
						done
					done
...
sbatch --array=1-${LINE_COUNT} submit_jobs_convnet.sbatch ${ARRAY_INDEX} ${PARAM_FILE}