...
def make_manifold_data(dataset, sampled_classes, examples_per_class, max_class=None, seed=0):
    ...
    for i in idx:
        ...
        label_idx = np.where(label==1)[0][0]  # get the label index
        # print(label_idx)
        if label_idx in sampled_labels and len(sampled_data[label_idx]) < examples_per_class:
            sampled_data[label_idx].append(torch.tensor(sample))
    ...
    for s, d in sampled_data.items():
        # print('s:', s)
        # print('d shape in sampled data', d[0].shape)
